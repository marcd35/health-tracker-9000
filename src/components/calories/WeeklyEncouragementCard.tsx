...
export function WeeklyEncouragementCard({ weeklyData }: WeeklyEncouragementCardProps) {
  const message = useMemo(() => {
    ...
    const generateMessage = () => {
      ...
      if (daysMetGoal === 7) {
        ...
        const hash = daysMetGoal + (weeklyData.days?.length || 0); // simple hash
      ...
      <CardContent className="pt-4">
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            <p className={`${getTextColor()} text-sm font-medium leading-relaxed`}>{message}</p>
          </div>
        </div>
      </CardContent>
    </Card>
