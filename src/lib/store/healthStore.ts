...
export const useHealthStore = create<HealthState>((set, get) => {
  ...
  console.log('[DEBUG] HealthStore initialized with activeDate:', today);
  return {
    profile: null,
    preferences: null,
    dailyLog: null,
    ...
    activeDate: today, // Initialize to today
    ...
    fetchDailyLog: async (date: string) => {
      console.log('[DEBUG] fetchDailyLog called with date:', date);
      set({ isLoading: true, error: null });
      try {
        const response = await fetch(`/api/daily-summary/${date}`);
        console.log('[DEBUG] fetchDailyLog response status:', response.status);
        if (!response.ok) {
          if (response.status === 404) {
            // No data for this date - set empty daily log instead of error
            console.log('[DEBUG] fetchDailyLog: No data found for date:', date);
            set({ dailyLog: null, isLoading: false });
            return;
          }
          const errorData = await response.json().catch(() => ({}));
          console.log('[DEBUG] fetchDailyLog error data:', errorData);
          throw new Error('Failed to fetch daily log');
        }
        const data = await response.json();
        console.log('[DEBUG] fetchDailyLog success, data keys:', Object.keys(data));
        set({ dailyLog: data, isLoading: false });
      } catch (err: any) {
        console.error('[DEBUG] fetchDailyLog error:', err);
        set({ error: err.message, isLoading: false });
